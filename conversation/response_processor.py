...
- Uses FallbackClassifier to produce clean, categorised fallback messages
  instead of raw internal error strings.
...
# Shared classifier instance for the module
...
class ResponseProcessor:
    ...
    def process(self, brain_response: Any) -> ProcessedResponse:
        ...
        if not brain_response:
            fallback_result = _classifier.classify(category=CATEGORY_INVALID_RESPONSE)
            return ProcessedResponse(
                sanitized_content=fallback_result.user_message,
                is_safe=False,
        ...
        if not self.evaluate_safety(sanitized):
            fallback_result = _classifier.classify(
                raw_error=raw_content or "empty response",
